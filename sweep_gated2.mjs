...
for (const t of TARGETS) {
  ...
  try {
    ...
    // Empty page shows "Add section" button instead of a canvas — click it if present
    await page.locator('button:has-text("Add section")').click({ timeout: 3000 }).catch(() => {});
    await page.waitForTimeout(500);
    // Switch to the "Blocks" sub-tab (individual blocks, not pre-made sections)
    await page.locator('.border-b button:has-text("Blocks"), button:has-text("Blocks")').last().click({ timeout: 5000 });
    await page.waitForTimeout(400);
    const searchBox = page.locator('input[placeholder*="Search blocks" i]');
    await searchBox.fill(t.label, { timeout: 5000 });
    await page.waitForTimeout(600);
    await page.screenshot({ path: `${OUT}/gated2-${t.slug}-search.png` });
  ...
  await page.screenshot({ path: `${OUT}/gated2-${t.slug}-content.png` });
  await page.locator('button:has-text("Style")').first().click({ timeout: 3000 }).catch(() => {});
  await page.waitForTimeout(400);
  await page.screenshot({ path: `${OUT}/gated2-${t.slug}-style.png` });

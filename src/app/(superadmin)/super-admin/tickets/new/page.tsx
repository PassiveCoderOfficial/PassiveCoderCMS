      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
            ...
            {selected ? (
              <div className="flex items-center gap-3 bg-primary/10 border border-primary/20 rounded-lg px-3 py-2.5">
                <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                  {selected.tenant_slug ? <Globe className="w-3.5 h-3.5 text-primary" /> : <User className="w-3.5 h-3.5 text-primary" />}
                </div>
                ...
                <button onClick={clearSelection} className="text-muted-foreground hover:text-destructive p-1 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
              <div className="relative">
                    ...
                    {results.map(r => (
                      ...
                      >
                        <div className="w-8 h-8 rounded-lg bg-primary/10 border border-primary/10 flex items-center justify-center shrink-0 mt-0.5">
                          {r.tenant_slug ? <Globe className="w-3.5 h-3.5 text-primary" /> : <User className="w-3.5 h-3.5 text-primary" />}
                        </div>
                ...
                {query.length >= 2 && !searching && results.length === 0 && (
                  <div className="absolute z-20 w-full mt-1 bg-popover border rounded-xl px-4 py-3 text-sm text-muted-foreground">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Guest / Contact Email</Label>
          ...
          <div className="space-y-2">
            <Label>Screenshots (max {MAX_ATTACH} images)</Label>
            <div className="flex flex-wrap gap-2">
              {attachments.map((url, i) => (
                <div key={i} className="relative group w-20 h-20 rounded-lg overflow-hidden border bg-card">
                  ...
                  <button onClick={() => setAttachments(prev => prev.filter((_, j) => j !== i))}
                    className="absolute top-0.5 right-0.5 bg-black/70 hover:bg-destructive/80 rounded-full p-1 transition-colors">
              ...
              {attachments.length < MAX_ATTACH && (
                <button onClick={() => fileRef.current?.click()} disabled={uploading}
                  className="w-20 h-20 rounded-lg border-2 border-dashed hover:border-primary flex flex-col items-center justify-center gap-1 text-muted-foreground hover:text-primary transition-colors">

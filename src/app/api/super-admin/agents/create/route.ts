...
function nameToCode(name: string): string {
  return name.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z0-9]/g, "").slice(0, 20) || "staff";
}
...
export async function POST(req: Request) {
  ...
  const { full_name, email, password, company, website, bio, staff_recurring_pct, notes, existing_user_id } = body as {
    full_name: string;
    email: string;
    password?: string;
    company?: string;
  ...
  const { data: existing } = await supabase.from("pc_staff").select("id").eq("user_id", userId).maybeSingle();
  if (existing) {
    return NextResponse.json({ error: "This user is already staff" }, { status: 409 });
  }

  // Generate name-based referral code, ensure unique
  const referral_code = await uniqueCode(supabase, nameToCode(full_name.trim()));

  const { data: agent, error: agentErr } = await supabase.from("pc_staff").insert({
    user_id: userId,
  ...
  // Set profile role to 'pc_staff' so they can access /dashboard
  await supabase.from("profiles").update({ role: "pc_staff" }).eq("id", userId);

...
export async function GET(req: Request) {
  ...
  const [plansResult, subsResult] = await Promise.all([
    ...
    id
      ? supabase!.from("subscriptions").select("*, tenants(name,slug,tenant_number)").eq("id", id).maybeSingle()
      : ticketId
        ? supabase!.from("subscriptions").select("*, tenants(name,slug,tenant_number)").eq("manual_ticket_id", ticketId).maybeSingle()
        : supabase!.from("subscriptions").select("*, tenants(name,slug,tenant_number)").order("created_at", { ascending: false }).limit(500),
  ]);
...
export async function POST(req: Request) {
  ...
  const payload: Record<string, unknown> = { tenant_id, plan_id, status: status ?? "onboarded", currency: currency || "USD" };
...
export async function PATCH(req: Request) {
  ...
  const allowed = [
    "plan_id","status","payment_provider","billing_cycle","amount_cents","currency",
    "custom_amount_cents","discount_pct","notes","next_payment_due","last_paid_at",

...
function CTAGradientBanner({ data }: { data: CTABlockProps["data"] }) {
  return (
    <div className="max-w-5xl mx-auto text-center bg-gradient-to-r from-primary to-secondary rounded-2xl px-8 py-14 text-white shadow-xl shadow-primary/20">
      ...
      {data.description && <p className="text-white/80 text-lg mb-8 max-w-2xl mx-auto">{data.description}</p>}
...
function CTADarkSplit({ data }: { data: CTABlockProps["data"] }) {
  return (
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 bg-card border border-border rounded-xl px-10 py-10">
      <div className="max-w-xl">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide">{data.title}</h2>
        {data.description && <p className="text-muted-foreground mt-3 text-sm leading-relaxed">{data.description}</p>}
      </div>
...
function CTANavyBanner({ data }: { data: CTABlockProps["data"] }) {
  return (
    <div className="max-w-5xl mx-auto bg-primary rounded-none px-10 py-14 text-primary-foreground flex flex-col md:flex-row items-center justify-between gap-8">
      <div>
        ...
        {data.description && <p className="text-primary-foreground/80 max-w-xl text-sm leading-relaxed">{data.description}</p>}
      </div>
      <div className="shrink-0">
        <CTAButtons data={data} dark />
      </div>
...
function CTAWarmBanner({ data }: { data: CTABlockProps["data"] }) {
  return (
    <div className="max-w-4xl mx-auto text-center bg-primary/10 border border-primary/20 rounded-xl px-8 py-12">
      <h2 className="text-3xl font-bold italic mb-3">{data.title}</h2>
      {data.description && <p className="text-muted-foreground mb-8 max-w-xl mx-auto text-sm leading-relaxed">{data.description}</p>}
      <CTAButtons data={data} />
    </div>
  );
}
...
function CTAOrangeBanner({ data }: { data: CTABlockProps["data"] }) {
  return (
    <div className="max-w-6xl mx-auto cta-section bg-gradient-to-r from-primary to-secondary rounded-none px-8 py-12 text-white">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight">{data.title}</h2>
          {data.description && <p className="text-white/80 mt-2 text-sm">{data.description}</p>}
        </div>
        <div className="shrink-0">
          <CTAButtons data={data} dark />
        </div>
...
export function CTABlock({ block }: { block: CTABlockProps }) {
  ...
  if (variant === "gradient-banner") return <CTAGradientBanner data={block.data} />;
  if (variant === "dark-split") return <CTADarkSplit data={block.data} />;
  if (variant === "navy-banner") return <CTANavyBanner data={block.data} />;
  if (variant === "warm-banner") return <CTAWarmBanner data={block.data} />;
  if (variant === "orange-banner") return <CTAOrangeBanner data={block.data} />;

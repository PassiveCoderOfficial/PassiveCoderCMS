...
const STAFF_SECTIONS: ShellNavSection[] = [
  {
    label: "Staff Portal",
    items: [
...
function StaffHeader({ agent }: { agent: Staff }) {
  return (
    <>
...
function StaffFooter({ sites }: { sites: Site[] }) {
...
export default function StaffSidebar({ agent, sites }: { agent: Staff; sites: Site[] }) {
  return (
    <Shell
      ...
      header={<StaffHeader agent={agent} />}
      footer={<StaffFooter sites={sites} />}
    />
  );
}
